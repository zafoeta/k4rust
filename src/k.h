...
#endif
#if !defined(KXVER)
// Asof 2023.03.30, default to 3. For kdb+2.x or standalone c-api before
// 2011-04-20 used KXVER=2
#define KXVER 3
#endif
#if KXVER >= 3
typedef struct k0 {
...
#endif
extern J gc(J j);
extern I khpunc(const S, I, const S, I, I), khpun(const S, I, const S, I),
    khpu(const S, I, const S), khp(const S, I), okx(K), ymd(I, I, I), dj(I);
extern V r0(K), sd0(I), sd0x(I d, I f), kclose(I);
extern S sn(const S, I), ss(const S);
extern K ee(K), ktj(I, J), ka(I), kb(I), kg(I), kh(I), ki(I), kj(J), ke(F),
    kf(F), kc(I), ks(const S), kd(I), kz(F), kt(I), sd1(I, K (*)(I)),
    dl(V *f, J), m4(I), knk(I, ...), kp(const S), ja(K *, V *), js(K *, S),
    jk(K *, K), jv(K *k, K), k(I, const S, ...), xT(K), xD(K, K), ktd(K), r1(K),
    krr(const S), orr(const S), dot(K, K), b9(I, K), d9(K), sslInfo(K x),
    vaknk(I, va_list), vak(I, const S, va_list), vi(K, UJ), vk(K);
#ifdef __cplusplus
}
#endif
